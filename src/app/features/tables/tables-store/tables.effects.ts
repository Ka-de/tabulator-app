...
import { ColumnService } from "@app/features/columns/columns.service";
import { PopupService } from "@app/features/popup";
import { RowService } from "@app/features/rows/rows.service";
...
export class TablesEffects {
    ...
    createTableColumn$: Observable<TablesAction> = this.action$.pipe(
        ...
        tap(() => loading()),
        tap(() => this.store.dispatch(new RemoveError)),
        mergeMap((action: CreateTableColumn) => this.columnsService.createColumn(action.payload._id, action.payload.data).pipe(
            tap(() => loaded()),
            map(column => {
                this.toastService.showMessage({
                    title: 'Create column',
    ...
    updateTableColumn$: Observable<TablesAction> = this.action$.pipe(
        ...
        tap(() => loading()),
        tap(() => this.store.dispatch(new RemoveError)),
        mergeMap((action: UpdateTableColumn) => this.columnsService.updateColumn(action.payload._id, action.payload.column_id, action.payload.data).pipe(
            tap(() => loaded()),
            map(column => {
                this.toastService.showMessage({
                    title: 'Update column',
    ...
    cloneTableColumn$: Observable<TablesAction> = this.action$.pipe(
        ...
        tap(() => loading()),
        tap(() => this.store.dispatch(new RemoveError)),
        mergeMap((action: CloneTableColumn) => this.columnsService.cloneColumn(action.payload._id, action.payload.data).pipe(
            tap(() => loaded()),
            map(table => {
                this.toastService.showMessage({
                    title: 'Clone column',
    ...
    deleteTableColumn$: Observable<TablesAction> = this.action$.pipe(
        ...
        tap(() => loading()),
        tap(() => this.store.dispatch(new RemoveError)),
        mergeMap((action: DeleteTableColumn) => this.columnsService.deleteColumn(action.payload._id, action.payload.column_id).pipe(
            tap(() => loaded()),
            map(data => {
                this.toastService.showMessage({
                    type: 'success',
    ...
    createTableRow$: Observable<TablesAction> = this.action$.pipe(
        ...
        tap(() => loading()),
        tap(() => this.store.dispatch(new RemoveError)),
        mergeMap((action: CreateTableRow) => this.rowsService.createRow(action.payload._id, action.payload.data).pipe(
            tap(() => loaded()),
            map(row => {
                this.toastService.showMessage({
                    title: 'Create row',
    ...
    updateTableRow$: Observable<TablesAction> = this.action$.pipe(
        ...
        tap(() => loading()),
        tap(() => this.store.dispatch(new RemoveError)),
        mergeMap((action: UpdateTableRow) => this.rowsService.updateRow(action.payload._id, action.payload.row_id, action.payload.data).pipe(
            tap(() => loaded()),
            map(row => {
                this.toastService.showMessage({
                    title: 'Update row',
    ...
    deleteTableRow$: Observable<TablesAction> = this.action$.pipe(
        ...
        tap(() => loading()),
        tap(() => this.store.dispatch(new RemoveError)),
        mergeMap((action: DeleteTableRow) => this.rowsService.deleteRow(action.payload._id, action.payload.row_id).pipe(
            tap(() => loaded()),
            map(data => {
                this.toastService.showMessage({
                    type: 'success',
